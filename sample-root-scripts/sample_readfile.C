...
void sample_readfile(char *filename=NULL, bool verbose=false)
{
  ...
  */
  ...
  TH1F *hE = new TH1F("hE", "Energy of neutron;E_{n0}", 100, 0, 20);
  
  int num_trig=0;
  
  TFile f(TString::Format("%s_plots.root", filename),"RECREATE");
  TTree * t4 = new TTree("truehit", "");
  TTree * t3 = new TTree("hit", "");
  TTree * t = new TTree("event", "");
  TTree * t2 = new TTree("global", "");
  double tE, tX, tY, tZ, tQ, tM;
  int tNDigi20, tNEv20, tPDG, tParentId, tPMTId, tNPhotonsPerDigiHit;
  int tPMTIdTrue, tNTrue20;
  float tNEv;
  int tEv, tTrueTime;
  t->Branch("E", &tE);
  t->Branch("M", &tM);
  t->Branch("PDG", &tPDG);
  t->Branch("ParentId", &tParentId);
  t->Branch("X", &tX);
  t->Branch("Y", &tY);
  t->Branch("Z", &tZ);
  t->Branch("NDigi20", &tNDigi20);
  t->Branch("NTrue20", &tNTrue20);
  t2->Branch("NEv", &tNEv);
  t2->Branch("NEv20", &tNEv20);
  t3->Branch("PMTId", &tPMTId);
  t3->Branch("Q", &tQ);
  t3->Branch("NPhotonsPerDigiHit", &tNPhotonsPerDigiHit);
  t4->Branch("PMTId_True", &tPMTIdTrue);
  t4->Branch("TrueTime", &tTrueTime);
  t4->Branch("Ev", &tEv);
  ...
  for (int ev=0; ev<nevent; ev++)
  {
    ...
    // tX = wcsimrootevent->GetVtx(0);
    // tY = wcsimrootevent->GetVtx(1);
    // tZ = wcsimrootevent->GetVtx(2);
    ...
    // if(ntrack){
    //   TObject *element = (wcsimrootevent->GetTracks())->At(3);
    //   WCSimRootTrack *wcsimroottrack = dynamic_cast<WCSimRootTrack*>(element);
    //   tE = wcsimroottrack->GetE();
    //   tM = wcsimroottrack->GetM();
    //   tPDG = wcsimroottrack->GetIpnu();
    //   tParentId = wcsimroottrack->GetParenttype();
    //   int itrack = 3;
    //   if(!verbose_track){
    //     double KE = tE - tM;
    //     hE->Fill(KE);
    ...
    // 	  printf("Track mass: %f\n", wcsimroottrack->GetM());
    // 	  for (int j=0; j<3; j++)
    //         printf("Vertex position: %f\n", wcsimroottrack->GetStart(j));
    //   }
    ...
    int ncherenkovhits     = wcsimrootevent->GetCherenkovHitTimes()->GetEntries();
    int ncherenkovdigihits = wcsimrootevent->GetNcherenkovdigihits(); 

    std::cout << "wcsimrootevent->GetNcherenkovhits() = " << ncherenkovhits << " | wcsimrootevent->GetCherenkovHits()->GetEntries() = " << wcsimrootevent->GetCherenkovHits()->GetEntries() << std::endl;
    std::cout << "wcsimrootevent->GetNcherenkovdigihits() = " << ncherenkovdigihits << " | wcsimrootevent->GetCherenkovDigiHits()->GetEntries() = "<< wcsimrootevent->GetCherenkovDigiHits()->GetEntries() << std::endl;
    
    h1->Fill(ncherenkovdigihits);
    if(ncherenkovdigihits > 0)
      num_trig++;
    tNTrue20 = ncherenkovhits;
    tNDigi20 = ncherenkovdigihits;
    t->Fill();
    if(verbose){
      printf("node id: %i\n", ev);
    ...
    //   tPMTIdTrue = tubeNumber;
     
    //   if(i < 10 && verbose) printf("Total pe: %d times( ",peForTube);
    ...
    //       if(i < 10 && verbose) printf("%6.2f ", HitTime->GetTruetime() );
    //       tTrueTime = HitTime->GetTruetime();
    //       t4->Fill();
    //     }
    //   if(i < 10 && verbose) cout << ")" << endl;
    ...
    // for (int index = 0 ; index < 1; index++) //if not using NoTrigger: index < wcsimrootsuperevent->GetNumberOfEvents() 
    // {
    ...
    //   if(verbose) printf("Ncherenkovdigihits %d\n", ncherenkovdigihits);
      ...
      // 	}
      // 	tPMTId = wcsimrootcherenkovdigihit->GetTubeId();
      //   tQ = wcsimrootcherenkovdigihit->GetQ();
      //   tNPhotonsPerDigiHit = wcsimrootcherenkovdigihit->GetPhotonIds().size();
      //   t3->Fill();
  ...
  TCanvas * c2 = new TCanvas("c2", "First canvas", 500*n_wide*win_scale, 500*n_high*win_scale);
  c2->Draw();
  c2->Divide(2,2);
  c2->cd(1); hE->Draw();
  c2->cd(2); //hvtx1_mpmt->Draw();
  c2->cd(3); //->Draw();
  c2->cd(4); //h1_mpmt->Draw();
  ...
  if(verbose) {
    cout << "num evts: " << nevent << endl;
    cout << "entries in histograms: " << endl
         << "h1 " << h1->GetEntries() << endl
         << "hvtx0 " << hvtx0->GetEntries() << endl
         << "hvtx1 " << hvtx1->GetEntries() << endl
         << "hvtx2 " << hvtx2->GetEntries() << endl
         << "hE " << hE->GetEntries() << endl;
